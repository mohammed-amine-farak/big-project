...
<div class="p-8">
    <div class="max-w-2xl mx-auto">
        <div class="mb-6">
            <h1 class="text-2xl font-bold text-gray-800">✏️ تعديل المادة</h1>
            <p class="text-gray-600 text-sm mt-1">تعديل بيانات المادة الدراسية</p>
        </div>

        <form action="{{ route('admin.subjects.update', $subject) }}" method="POST" class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            @csrf
            @method('PUT')
                ...
                <div>
                    ...
                    <select name="fields_id" 
                        ...
                        @foreach($fields as $field)
                            <option value="{{ $field->id }}" {{ old('fields_id', $subject->fields_id) == $field->id ? 'selected' : '' }}>
