...
<div class="p-8">
    <div class="max-w-4xl mx-auto">
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-4xl font-bold text-gray-800">إضافة امتحان أسبوعي جديد</h1>
        ...
        <div class="bg-white rounded-2xl shadow-xl overflow-hidden p-8">
            <form action="{{ route('exam_weeklies.store') }}" method="POST" enctype="multipart/form-data">
                @csrf
...
@endforeach
                ...
                <div class="mb-6">
                    ...
                    <select id="classroom_id" name="classroom_id" required
                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-base p-3 transition duration-150">
                        <option value="" class="text-gray-400">-- اختر المادة أولاً --</option>
                        {{-- Classrooms will be loaded via AJAX after subject selection --}}
                    </select>
                    @error('classroom_id')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                        ...
                        <div class="space-y-1 text-center">
                            ...
                            <div class="flex text-sm text-gray-600">
                                <label for="file_path" class="relative cursor-pointer bg-white rounded-md font-medium text-blue-600 hover:text-blue-500 focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-blue-500">
                                    <span>اختر ملف</span>
                                    <input id="file_path" name="file_path" type="file" class="sr-only" accept=".pdf,.doc,.docx,.jpg,.jpeg,.png" required>
                                </label>
...
$(document).ready(function() {
    ...
    $('#subject_id').change(function() {
        ...
        if (!subjectId) {
            ...
            classroomSelect.html('<option value="" class="text-gray-400">-- اختر المادة أولاً --</option>');
            ...
            success: function(data) {
                ...
                if (data.classrooms && data.classrooms.length > 0) {
                    ...
                    data.classrooms.forEach(function(classroom) {
                        const selected = (classroom.id == '{{ old('classroom_id') }}') ? 'selected' : '';
    ...
    // Trigger change on page load if subject is preselected
    @if(old('subject_id'))
        console.log('Loading classrooms for preselected subject');
