        ...
        <div class="mb-8">
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div class="flex-1 min-w-0">
                    <h1 class="text-2xl sm:text-3xl font-bold text-gray-900 mb-2">إنشاء اختبار جديد</h1>
                    <p class="text-gray-600 text-sm">قم بإنشاء اختبار شامل مع الأسئلة والخيارات المتعددة</p>
                </div>
        ...
        <form action="{{ route('exam.store') }}" method="POST" class="space-y-6">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                ...
                <div id="questions-container" class="p-6 space-y-6">
                    <!-- Initial Question -->
                    <div id="question-1" class="question-block bg-gray-50 rounded-lg border-2 border-dashed border-gray-300 p-6">
                        <div class="flex items-center justify-between mb-4">
                            <div class="flex items-center gap-3">
                                <span class="bg-blue-100 text-blue-800 px-3 py-1 rounded-full text-sm font-medium">سؤال 1</span>
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="questions[0][text]" placeholder="اكتب نص السؤال هنا..." rows="3" 
                        ...
                        <div id="choices-container-1" class="space-y-3">
                            <div class="flex items-center justify-between">
                            ...
                            <div class="choice-block bg-white p-4 rounded-lg border border-gray-200">
                                <div class="flex items-center gap-3">
                                    <div class="flex items-center gap-2 flex-1">
                                        ...
                                        <input type="text" name="questions[0][choices][0][text]" placeholder="أدخل نص الخيار..." 
                                    ...
                                    <label class="flex items-center gap-2 cursor-pointer">
                                        <input type="radio" name="questions[0][correct_choice]" value="0" 
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const createQuestionTemplate = (questionIndex) => {
        ...
        newQuestion.classList.add('question-block', 'bg-gray-50', 'rounded-lg', 'border-2', 'border-dashed', 'border-gray-300', 'p-6');
        newQuestion.id = `question-${questionIndex + 1}`;
        newQuestion.setAttribute('draggable', 'true');
        
        newQuestion.innerHTML = `
            <div class="flex items-center justify-between mb-4">
                <div class="flex items-center gap-3">
                    <span class="bg-blue-100 text-blue-800 px-3 py-1 rounded-full text-sm font-medium">سؤال ${questionIndex + 1}</span>
                    <span class="text-sm text-gray-500">اسحب لنقل الترتيب</span>
                </div>
                <button type="button" class="remove-question-btn text-red-600 hover:text-red-700 font-medium text-sm flex items-center gap-1 transition duration-200">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="mb-4">
                ...
                <textarea name="questions[${questionIndex}][text]" placeholder="اكتب نص السؤال هنا..." rows="3" 
                          class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition duration-200"></textarea>
            </div>

            <div id="choices-container-${questionIndex + 1}" class="space-y-3">
                <div class="flex items-center justify-between">
                    <h4 class="text-sm font-medium text-gray-700">خيارات الإجابة</h4>
                    <span class="text-xs text-gray-500">حدد الإجابة الصحيحة</span>
                </div>
        ...
        // Add initial choices
        const choicesContainer = newQuestion.querySelector(`#choices-container-${questionIndex + 1}`);
        createChoiceTemplate(choicesContainer, questionIndex, 0);
        createChoiceTemplate(choicesContainer, questionIndex, 1);
    ...
    // Simple drag and drop for questions
