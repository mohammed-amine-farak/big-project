...
<div class="flex-1 p-6 lg:p-10 main-content-area">
    <div class="main-content-container">
        <h2 class="text-3xl font-bold mb-8 text-blue-900">إنشاء درس جديد</h2>

        <form method="POST" action="{{ route('lessons.store') }}"  class="space-y-6 bg-white p-6 rounded-lg shadow-md">
            @csrf
            <div>
                <label for="title" class="block text-sm font-medium text-gray-700 mb-1">عنوان الدرس</label>
                <input type="text" id="title" name="title" required class="w-full border border-gray-300 rounded-md px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-400">
            </div>

            <div>
                <label for="description" class="block text-sm font-medium text-gray-700 mb-1">وصف الدرس</label>
                <textarea id="description" name="content" rows="4" class="w-full border border-gray-300 rounded-md px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-400"></textarea>
            </div>
            ...
            <div>
                ...
                @foreach ($subjects as $subject)
                    <option value="{{ $subject->subjects_id }}" class="text-gray-700">
                        {{ $subject->subject_name }} - {{ $subject->field_name }} - {{ $subject->study_level }}
                    </option>
                @endforeach
            </select>
            </div>
