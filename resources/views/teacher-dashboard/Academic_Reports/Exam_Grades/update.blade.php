    ...
    <div class="max-w-7xl mx-auto">
        ...
        <div class="bg-green-100 border-r-4 border-green-500 text-green-700 p-4 mb-4 shadow-md rounded-lg" role="alert">
            <div class="flex items-center">
                ...
                <div>
                    <p class="font-bold">تم تحديث التقرير بنجاح!</p>
                </div>
            </div>
        </div>

        <div class="bg-white rounded-2xl shadow-xl overflow-hidden p-8">
            <form action="{{ route('Exam_Grade.update',$exam_grade->exam_schol_weeckly_reports_id) }}" method="POST">
                @method('PUT')
                @csrf    
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select id="student_id" name="student_id" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm p-2 transition duration-150">
                            <option value="{{ $exam_grade->student_id}}">{{ $exam_grade->student_name}}</option>
                    ...
                    <div>
                        ...
                        <select id="exam_weecklies_id" name="exam_weecklies_id" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm p-2 transition duration-150">
                            <option value="{{ $exam_grade->exam_weeckly_id}}">{{ $exam_grade->exam_weeckly_title}}</option>
                ...
                <div class="mt-6">
                    ...
                    <input value="{{ $exam_grade->exam_weeckly_total_point}}" type="number" name="exam_total_point" id="exam_total_point" min="0" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm p-2 transition duration-150" placeholder="أدخل مجموع النقاط">
                </div>

                <!-- حقل ملاحظات الأستاذ -->
                <div class="mt-6">
                    <label for="exam_note" class="block text-sm font-medium text-gray-700 mb-1">ملاحظات الأستاذ</label>
                    <textarea aria-valuemax="" id="exam_note" name="exam_note" rows="4" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm p-2 transition duration-150" placeholder="أضف ملاحظاتك حول أداء التلميذ في هذا الاختبار...">{{ $exam_grade->exam_weeckly_note}}</textarea>
