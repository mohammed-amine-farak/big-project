    ...
    <div class="max-w-4xl mx-auto bg-white rounded-2xl shadow-xl">
        <div class="p-8">
            ...
            <div class="flex justify-between items-center mb-6">
                <h1 class="text-3xl font-bold text-gray-800">إضافة ملاحظة تفاعل جديدة 📝</h1>
                <a href="{{ url()->previous() }}" class="text-blue-600 hover:text-blue-800 font-medium">
                    &larr; العودة
                </a>
            </div>

            <form action="{{route('Interaction_Notes_student.store')}}" method="POST" class="space-y-6">
                @csrf
                ...
                <div>
                    ...
                    <textarea id="note_content" name="note_content" rows="6" required
                              class="w-full border border-gray-300 rounded-md px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-400"
                              placeholder="اكتب ملاحظاتك حول تفاعل الطالب في هذا الدرس هنا...">{{ old('note_content') }}</textarea>
                    @error('note_content')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
